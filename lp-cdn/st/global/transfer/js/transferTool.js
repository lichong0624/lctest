...
layui.define(['element', 'form',], function (exports) {
    ...
    var transferTool = {
        ...
        renderData     : function (R) {
            ...
            var rightList = "";
            //数据的key
            var _key      = config.keyName;
            var _item     = config.valueName;

            if (data !== undefined) {
                for (var i = 0; i < data.length; i++) {
                    //设置默认值(向右侧插入元素)
                    if (_values.indexOf(data[i][_key].toString()) == -1) {
                        var _input = '<dd lay-value="' + data[i][_key] + '" lay-title="' + data[i][_item] + '"><input type="checkbox" lay-filter="transferLeftChecked" title="' + data[i][_item] + '" lay-skin="primary"></dd>';
                        //设置禁用
                        if (_disableds.indexOf(data[i][_key].toString()) != -1) {
                    ...
                    //像左侧插入元素
                        ...
                        var _input = '<dd lay-value="' + data[i][_key] + '"  lay-title="' + data[i][_item] + '"><input type="hidden" name="' + _name + '" value="' + data[i][_key] + '"><input lay-filter="transferRightChecked"   type="checkbox"  title="' + data[i][_item] + '" lay-skin="primary"></dd>';
                        //设置禁用
                        if (_disableds.indexOf(data[i][_key].toString()) != -1) {
                            _input = _input.replace("<input", "<input disabled ")
                        }
            ...
            var outHtml =
                    $(config.elem).html([
                        '<div class="transfer-content" style="width: 600px;height: 340px;position: relative;margin-left: 150px;">',
