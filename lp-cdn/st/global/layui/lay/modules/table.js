...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  ,ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header',ELEM_FOOTER = '.layui-table-footer', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOOL = '.layui-table-tool', ELEM_PAGE = '.layui-table-page', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover'
    ...
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
      ...
      ,'{{# layui.each(d.data.cols, function(i1, item1){ }}'
        ...
        ,'{{# layui.each(item1, function(i2, item2){ }}'
          ...
          ,'<th data-sort-action="{{ item2.sortUrl }}" data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
  ...
  ,TPL_FOOTER = function(options){
      ...
      return options = options || {},
       ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table">'
          ,'<tfoot class="layui-table-footer">'
          ,'{{# layui.each(d.data.footData, function(i1, item1){ }}'
          ,'<tr class="row-footer">'
          ,'{{# layui.each(item1, function(i2, item2){ }}'
          ,'{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}'
          ,'{{# if(item2.fixed === "right"){ right = true; } }}'
          ...
          ,'<td {{# if (item2.isToal){}} class="row-footer" {{# } }} '+ rowCols +' >'
          ,'<div class="layui-table-cell laytable-cell-footer-{{d.index}}-{{item2.field || i2}}" {{# if(item2.align){}}align="{{item2.align}}"{{#}}}>'
          ,'{{# if(item2.templet){  }}'
          　　,"{{# item2.value = layui.laytpl($(item2.templet).html()||'').render(item2); }}"
          ,'{{# } }}'
  ...
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'{{# layui.each(d.data.footData, function(i1, item1){'
      ,'layui.each(item1, function(i2, item2){ }}'
        ,'.laytable-cell-footer-{{d.index}}-{{item2.field||i2}}{ '
        ,'{{# if(item2.width){ }}'
  ...
  Class.prototype.render = function(){
    ...
    that.layFooter = reElem.find(ELEM_FOOTER);
  ...
  Class.prototype.setArea = function(){
    ...
    //遍历所有列footData
    layui.each(options.footData, function(i1, item1){
      layui.each(item1, function(i2, item2){
          var width;
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          //自定义属性
          $.each(item3, function (_cols_i, _custom) {
              if ($.inArray(_cols_i, ["edit", "align", "templet", "toolbar", "event", "style", "minWidth"]) === -1) {
                  if (_custom) {
                      attr.push('data-' + _cols_i + '=' + _custom);
                  }
              }
          });
  ...
  Class.prototype.scrollPatch = function(){
    ...
    if(scollWidth && scollHeight){
      if(!that.elem.find('.layui-table-patch')[0]){
        ...
        that.layFooter.eq(2).find('tfoot tr').append(patchElem.clone())
      }
    } else {
