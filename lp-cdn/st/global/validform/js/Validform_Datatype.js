...
(function(){
	if($.Datatype){
		$.extend($.Tipmsg.w,{
			...
			"isDecimals2":"小数点最多两位",
			"isDecimals3":"小数点最多三位",
			"isPassword":"请输入8~20位字母数字组合",
			"emptyOrIsDecimals2":"小数点最多两位",
			"emptyOrIsDecimals3":"小数点最多三位"
		});
		...
		$.extend($.Datatype,{
			...
			*/
			...
			//非必填或小数点最多两位
			"emptyOrIsDecimals2":function(gets,obj,curform,datatype){
				gets = $.trim(gets);

				//console.log("emptyOrIsDecimals2 gets:【" +gets+ "】");
				
				if(gets && !(/^\d+\.?\d{0,2}$/.test(gets))){
					return false ;
				}
			...
			//非必填或小数点最多三位
			"emptyOrIsDecimals3":function(gets,obj,curform,datatype){
				gets = $.trim(gets);
				if(gets && !(/^\d+\.?\d{0,3}$/.test(gets))){
					return false ;
				}
