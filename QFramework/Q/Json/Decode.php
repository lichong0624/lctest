...
class Q_Json_Decoder
{
    ...
    protected function __construct($source, $decodeType)
    {
        ...
        if (!in_array($decodeType, array(Q_Json::TYPE_ARRAY, Q_Json::TYPE_OBJECT)))
        {
            $decodeType = Q_Json::TYPE_ARRAY;
        }
     ...
     *
     * Throws a Q_Exception if the source string is null.
     *
     * @static
     * @access public
     ...
     * @throws Q_Exception
     */
    public static function decode($source = null, $objectDecodeType = Q_Json::TYPE_ARRAY)
    {
        if (null === $source) {
            throw new Q_Exception('Must specify JSON encoded source for decoding');
        } elseif (!is_string($source)) {
            throw new Q_Exception('Can only decode JSON encoded strings');
        }
    ...
    protected function _decodeObject()
    {
        ...
        while ($tok && $tok != self::RBRACE) {
            if ($tok != self::DATUM || ! is_string($this->_tokenValue)) {
                throw new Q_Exception('Missing key in object encoding: ' . $this->_source);
            }
            ...
            if ($tok != self::COLON) {
                throw new Q_Exception('Missing ":" in object encoding: ' . $this->_source);
            }
    ...
    protected function _decodeArray()
    {
        ...
        while ($tok && $tok != self::RBRACKET) {
            ...
            if ($tok != self::COMMA) {
                throw new Q_Exception('Missing "," in array encoding: ' . $this->_source);
            }
    ...
    protected function _getNextToken()
    {
                    ...
                    if ($chr == '\\') {
                        ...
                        switch ($chr) {
                            ...
                            case '\'' :
                                ...
                                break;
                            default:
                                throw new Q_Exception("Illegal escape "
                                    .  "sequence '" . $chr . "'");
                            }
                    } elseif ($chr == '"') {
                        break;
        ...
        if ($chr == '-' || $chr == '.' || ($chr >= '0' && $chr <= '9')) {
            if (preg_match('/-?([0-9])*(\.[0-9]*)?((e|E)((-|\+)?)[0-9]+)?/s',
                ...
                if (is_numeric($datum)) {
                    if (preg_match('/^0\d+$/', $datum)) {
                        throw new Q_Exception("Octal notation not supported by JSON (value: $datum)");
                    } else {
                        $val  = intval($datum);
                        $fVal = floatval($datum);
                    ...
                    throw new Q_Exception("Illegal number format: $datum");
                }
            ...
            throw new Q_Exception('Illegal Token');
        }
